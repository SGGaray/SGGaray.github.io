...
const LoadingScreen = ({ onComplete }) => {
  const seen = typeof window !== 'undefined'
    ...
    && sessionStorage.getItem(LS_KEY) === '1';
  ...
  useEffect(() => {
    if (seen) { onComplete && onComplete(); return; }
    ...
    const doneTimer = setTimeout(() => { setMounted(false); onComplete && onComplete(); }, 840);

    return () => { cancelAnimationFrame(raf); clearTimeout(fadeTimer); clearTimeout(doneTimer); };
  }, []);
