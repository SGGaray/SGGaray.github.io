...
const ARC_PAIRS = [
  [0, 2], // Buenos Aires → London
  [0, 1], // Buenos Aires → New York
  [0, 6], // Buenos Aires → São Paulo
...
const slerp3D = (a, b, t) => {
  ...
  if (Math.abs(omega) < 1e-4) return { ...a };
...
class NetworkGlobe {
  ...
  _genPoints(n) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const phi = golden * i;
      pts.push({ x: Math.cos(phi)*rr, y, z: Math.sin(phi)*rr });
    }
    return pts;
  }
  ...
  _genConns() {
    ...
    for (let i = 0; i < this.points.length; i++) {
      for (let j = i+1; j < this.points.length; j++) {
        ...
        if (d > 0.922) { conns.push([i,j]); counts[i]++; counts[j]++; }
      }
    }
    return conns;
  }
  ...
  _rot(p) {
    const cy = Math.cos(this.rotation), sy = Math.sin(this.rotation);
    const cx = Math.cos(this.tilt),     sx = Math.sin(this.tilt);
    const rx = p.x*cy + p.z*sy;
    const ry0 = p.y, rz0 = -p.x*sy + p.z*cy;
    return { x: rx, y: ry0*cx - rz0*sx, z: ry0*sx + rz0*cx };
  }

  _proj(p, cx, cy, r) {
    const rp = this._rot(p);
    const s  = 4.5 / (4.5 + rp.z);
    return { sx: rp.x*s*r+cx, sy: rp.y*s*r+cy, z: rp.z };
  }
  ...
  draw() {
    ...
    proj.map((p,i) => ({...p,i})).sort((a,b) => a.z-b.z).forEach(({sx,sy,z,i}) => {
      if (z < -0.56) return;
      const t = (z+1)/2;
      ...
      if (t > 0.38) {
        ...
        ctx.beginPath(); ctx.arc(sx,sy,gr,0,Math.PI*2); ctx.fillStyle=g2; ctx.fill();
      }
    ...
    this.arcs.forEach(arc => {
      const fp = this.cityPts[arc.from], tp = this.cityPts[arc.to];
      ...
      for (let k = 0; k <= arc.segs; k++) {
        ...
        if (rp.z < -0.32) { aPts.push(null); continue; }
        const s = 4.5/(4.5+rp.z);
        aPts.push({ sx: rp.x*s*r+cx, sy: rp.y*s*r+cy, z: rp.z });
      }
      ...
      aPts.forEach(pt => {
        if (!pt) {
          if (drawing) { ctx.strokeStyle=`rgba(${rgb},0.14)`; ctx.lineWidth=0.75; ctx.stroke(); ctx.beginPath(); }
          drawing = false;
        } else {
      ...
      if (drawing) { ctx.strokeStyle=`rgba(${rgb},0.14)`; ctx.lineWidth=0.75; ctx.stroke(); }
      ...
      if (drp.z > -0.12) {
        ...
        const dsx = drp.x*ds*r+cx, dsy = drp.y*ds*r+cy;
        ...
        for (let k = 1; k <= 12; k++) {
          ...
          ctx.fillStyle = `rgba(${rgb},${da*(1-k/12)*0.5})`; ctx.fill();
        }
        // Dot glow
        ...
        ctx.beginPath(); ctx.arc(dsx,dsy,10,0,Math.PI*2); ctx.fillStyle=dg; ctx.fill();
        ctx.beginPath(); ctx.arc(dsx,dsy,2.2,0,Math.PI*2);
    ...
    this.cityPts.forEach((cp, i) => {
      ...
      if (rp.z < -0.06) return;
      const s  = 4.5/(4.5+rp.z);
      const sx = rp.x*s*r+cx, sy = rp.y*s*r+cy;
      const al = Math.min(1, (rp.z+0.5)/1.4);

      // Outer ring
      ctx.beginPath(); ctx.arc(sx,sy,8,0,Math.PI*2);
      ctx.strokeStyle=`rgba(${rgb},${al*0.35})`; ctx.lineWidth=0.8; ctx.stroke();

      // Glow
      const cg = ctx.createRadialGradient(sx,sy,0,sx,sy,16);
      cg.addColorStop(0, `rgba(${rgb},${al*0.42})`);
      cg.addColorStop(1, 'transparent');
      ctx.beginPath(); ctx.arc(sx,sy,16,0,Math.PI*2); ctx.fillStyle=cg; ctx.fill();

      // Dot
      ctx.beginPath(); ctx.arc(sx,sy,4.2,0,Math.PI*2);
      ctx.fillStyle=`rgba(${rgb},${al*0.9})`; ctx.fill();
      ctx.beginPath(); ctx.arc(sx,sy,1.8,0,Math.PI*2);
      ctx.fillStyle=`rgba(255,255,255,${al*0.75})`; ctx.fill();
  ...
  start() { const loop = () => { this.draw(); this.raf = requestAnimationFrame(loop); }; loop(); }
  stop()  { if (this.raf) { cancelAnimationFrame(this.raf); this.raf = null; } }
  setColor(c) { this.color = c; }
  setSpeed(s) { this.speed = s; }
}

const GlobeCanvas = memo(({ color='#00d4ff', speed=1 }) => {
  ...
  useEffect(() => {
    ...
    resize();
    const globe = new NetworkGlobe(canvas, { color, speed });
    globeRef.current = globe;
    globe.start();
    const ro = new ResizeObserver(resize);
    ro.observe(parent);
    return () => { globe.stop(); ro.disconnect(); };
  }, []);

  useEffect(() => { if (globeRef.current) globeRef.current.setColor(color); }, [color]);
  useEffect(() => { if (globeRef.current) globeRef.current.setSpeed(speed); }, [speed]);

  return <canvas ref={canvasRef} style={{ width:'100%', height:'100%', display:'block' }} />;
});

Object.assign(window, { GlobeCanvas, NetworkGlobe });
