...
const DynamicIsland = ({ accent = '#00d4ff' }) => {
  ...
  const LINKS = [
    {
      ...
      icon: (hov) => (
        <svg width="15" height="15" viewBox="0 0 24 24" fill={accent}
          style={{ display:'block', flexShrink:0, transition:'transform 0.22s cubic-bezier(0.34,1.56,0.64,1)', transform: hov ? 'translateY(-3px) scale(1.18)' : 'none' }}>
    ...
    },
    {
      ...
      icon: (hov) => (
        <GithubMark size={15} fill={accent}
          style={{ display:'block', flexShrink:0, transition:'transform 0.22s cubic-bezier(0.34,1.56,0.64,1)', transform: hov ? 'translateY(-3px) scale(1.18)' : 'none' }} />
      ),
    },
  ];
  ...
  return (
    <div
      ...
      style={{
        position: 'fixed', top: '76px', right: '20px', zIndex: 850,
        width: expanded ? '228px' : '84px', height: '38px',
        ...
        boxShadow: expanded
          ? `0 0 0 1px rgba(${rgb},0.06), 0 0 32px rgba(${rgb},0.08), 0 12px 40px rgba(0,0,0,0.5)`
          : `0 4px 14px rgba(0,0,0,0.35)`,
        cursor: 'default',
      }}
    >
      <div style={{ display:'flex', alignItems:'center', height:'100%', padding:'0 16px', gap:'0', whiteSpace:'nowrap' }}>
        ...
        {LINKS.map((link, i) => (
          ...
          >
            {link.icon(hoveredIdx === i)}
            <span style={{
              fontFamily: "'JetBrains Mono',monospace", fontSize: '11px',
              letterSpacing: '0.3px', color: `rgba(${rgb},0.8)`,
        ...
        <div style={{
          ...
          background: accent, boxShadow: `0 0 8px rgba(${rgb},0.8)`,
